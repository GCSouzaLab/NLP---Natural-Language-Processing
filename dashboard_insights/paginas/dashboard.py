"""
Página: Dashboard de Insights — Módulo 3 (Mayara).

4 abas: Origem (location→UF), Evolução temporal (date), Aspectos (ABSA Gemini)
e Nuvem de palavras. Os filtros (ano/praia/nota) ficam na barra lateral e valem
SÓ nesta página.

Esta página roda dentro do app multipage (app.py). Não chama set_page_config.
"""
import sys
from pathlib import Path

# garante que `import data` (data.py na pasta de cima) funcione ao rodar como página
...
def obter_dados():
    """Resolve a fonte do CSV: arquivo padrão (ao lado de data.py) ou upload."""
    caminho = Path(dados.__file__).resolve().parent / dados.ARQUIVO_PADRAO
    if caminho.exists():
...
st.caption("Módulo 3 · avaliações do TripAdvisor · aspectos via ABSA (Gemini)")
...
with aba_aspectos:
    ...
    st.caption("Aspectos do ABSA (Gemini) — matriz praia × aspecto, reagindo aos filtros.")
...
st.divider()
st.caption("Aspectos via ABSA (Gemini), ligados às avaliações por `review_id` em `data.py`.")
